---
home: true
title: WEBI
heroImage: https://0.gravatar.com/avatar/6d1b435a74677ddddc346307eed74660e1ba04f2ddaea5cb821e2ae39f413df0?s=400&d=mm
heroText: WEBI
navbar: false
sidebar: 'heading'
actions:
  - text: 胡田に連絡する
    link: mailto:ebibibi@gmail.com
    type: primary
---

<details class="about-me">
  <summary>胡田 昌彦（えびすだ まさひこ）について</summary>
  <div class="about-me-content">
    <p>1979年茨城県鹿嶋市生まれ。2002年3月に筑波大学第三学群情報学類を卒業後、都内の独立系SIerに入社。Windowsインフラを中心に、数人から数万人規模の企業システムの設計・構築・運用に従事。エンジニア向けの技術サポートやトレーニング、製品検証、技術的課題のあるプロジェクトにも直接参加し、多数のイベントで登壇。</p>
    <p>専門分野はMicrosoftサービス全般で、オンプレミス、クラウド、ハイブリッド環境からプログラミング、生成AIまで幅広くカバー。Microsoft MVPを2014年から連続受賞中。著書に「Windowsインフラ管理者入門」。現在はYouTubeチャンネルでも情報発信中。部下を持たずに複数のAIエージェントに仕事を依頼しながら新しい仕事スタイルに挑戦中。</p>
  </div>
</details>

<div class="sns-icons">
  <a href="https://x.com/ebi" target="_blank" class="sns-icon" title="X (Twitter)">
    <svg viewBox="0 0 24 24" fill="currentColor"><path d="M18.244 2.25h3.308l-7.227 8.26 8.502 11.24H16.17l-5.214-6.817L4.99 21.75H1.68l7.73-8.835L1.254 2.25H8.08l4.713 6.231zm-1.161 17.52h1.833L7.084 4.126H5.117z"/></svg>
  </a>
  <a href="https://www.facebook.com/masahiko.ebisuda/" target="_blank" class="sns-icon" title="Facebook">
    <svg viewBox="0 0 24 24" fill="currentColor"><path d="M24 12.073c0-6.627-5.373-12-12-12s-12 5.373-12 12c0 5.99 4.388 10.954 10.125 11.854v-8.385H7.078v-3.47h3.047V9.43c0-3.007 1.792-4.669 4.533-4.669 1.312 0 2.686.235 2.686.235v2.953H15.83c-1.491 0-1.956.925-1.956 1.874v2.25h3.328l-.532 3.47h-2.796v8.385C19.612 23.027 24 18.062 24 12.073z"/></svg>
  </a>
  <a href="https://www.linkedin.com/in/masahiko-ebisuda-8ab1712a/" target="_blank" class="sns-icon" title="LinkedIn">
    <svg viewBox="0 0 24 24" fill="currentColor"><path d="M20.447 20.452h-3.554v-5.569c0-1.328-.027-3.037-1.852-3.037-1.853 0-2.136 1.445-2.136 2.939v5.667H9.351V9h3.414v1.561h.046c.477-.9 1.637-1.85 3.37-1.85 3.601 0 4.267 2.37 4.267 5.455v6.286zM5.337 7.433c-1.144 0-2.063-.926-2.063-2.065 0-1.138.92-2.063 2.063-2.063 1.14 0 2.064.925 2.064 2.063 0 1.139-.925 2.065-2.064 2.065zm1.782 13.019H3.555V9h3.564v11.452zM22.225 0H1.771C.792 0 0 .774 0 1.729v20.542C0 23.227.792 24 1.771 24h20.451C23.2 24 24 23.227 24 22.271V1.729C24 .774 23.2 0 22.222 0h.003z"/></svg>
  </a>
</div>

<h2 class="section-heading">技術発信</h2>

<div class="card-grid">

<a href="https://ebibibi.github.io/yt-channel-navigator/" target="_blank" class="card">
  <div class="card-thumbnail">
    <img src="/images/yt-channel-navigator.png" alt="ナビゲーター">
  </div>
  <div class="card-content">
    <h3>エビちゃんねる ナビゲーター（動画ライブラリ）</h3>
    <p>全動画をカテゴリ・レベル別に閲覧＆字幕全文検索。キーワードで該当シーンから直接視聴できます。</p>
  </div>
</a>

<a href="https://www.youtube.com/@ebibibi" target="_blank" class="card">
  <div class="card-thumbnail">
    <img src="/images/youtube-ebibibi.png" alt="YouTube">
  </div>
  <div class="card-content">
    <h3>胡田(えびすだ)のコンピューター系チャンネル</h3>
    <p>Windows, Azure, Microsoft 365等を中心としたコンピューター系の情報を動画で発信しています。</p>
  </div>
</a>

<a href="https://discord.gg/h9eVunUEyU" target="_blank" class="card">
  <div class="card-thumbnail">
    <img src="/images/discord.png" alt="Discord">
  </div>
  <div class="card-content">
    <h3>Discord</h3>
    <p>Youtubeチャンネルを見て興味を持ってくれた人を中心に、チャットで簡単にコミュニケーションを取れるようになっています。</p>
  </div>
</a>

<a href="https://note.com/ebibibi" target="_blank" class="card">
  <div class="card-thumbnail">
    <img src="/images/note.png" alt="note">
  </div>
  <div class="card-content">
    <h3>胡田昌彦のnote</h3>
    <p>noteでは文字情報を発信しています。こちらもコンピューター系の情報が中心です。</p>
  </div>
</a>

<a href="https://speakerdeck.com/ebibibi" target="_blank" class="card">
  <div class="card-thumbnail">
    <img src="/images/speakerdeck.png" alt="Speaker Deck">
  </div>
  <div class="card-content">
    <h3>Speaker Deck（登壇スライド）</h3>
    <p>勉強会やイベントで登壇した際のスライドを公開しています。</p>
  </div>
</a>

<a href="https://amzn.to/3PvYqWG" target="_blank" class="card">
  <div class="card-thumbnail">
    <img src="/images/amazon-book.png" alt="書籍">
  </div>
  <div class="card-content">
    <h3>【書籍】Windows インフラ管理者入門</h3>
    <p>2014年2月に書籍を単著で出版しました。</p>
  </div>
</a>

<a href="https://mvp.microsoft.com/ja-JP/mvp/profile/959007f9-3c9a-e411-93f2-9cb65495d3c4" target="_blank" class="card">
  <div class="card-thumbnail">
    <img src="/images/mvp.png" alt="MVP">
  </div>
  <div class="card-content">
    <h3>Microsoft MVP プロファイル</h3>
    <p>胡田昌彦は2014年からMicrosoft MVPを10年以上連続受賞しています。</p>
  </div>
</a>

</div>

<h2 class="section-heading">OSS・開発プロジェクト</h2>

<div class="card-grid">

<a href="https://github.com/ebibibi/claude-code-discord-bridge" target="_blank" class="card">
  <div class="card-thumbnail">
    <img src="/images/oss-ccdb.png" alt="claude-code-discord-bridge">
  </div>
  <div class="card-content">
    <h3>claude-code-discord-bridge</h3>
    <p>DiscordからClaude Codeを操作するOSSフレームワーク。複数スレッドで並列開発セッションを実行できます。</p>
  </div>
</a>

<a href="https://github.com/ebibibi/marp2pptx" target="_blank" class="card">
  <div class="card-thumbnail">
    <img src="/images/oss-marp2pptx.png" alt="marp2pptx">
  </div>
  <div class="card-content">
    <h3>marp2pptx</h3>
    <p>MarpのMarkdownスライドを編集可能なPowerPointに変換。自動フォントサイズ調整・CJK対応。</p>
  </div>
</a>

<a href="https://github.com/ebibibi/process_skyblue" target="_blank" class="card">
  <div class="card-thumbnail">
    <img src="/images/oss-process-skyblue.png" alt="Process BlueSky">
  </div>
  <div class="card-content">
    <h3>Process BlueSky</h3>
    <p>BlueSkyからX (Twitter)・Discordへの自動クロスポストサービス。画像対応・スレッド分割・自動リトライ。</p>
  </div>
</a>

<a href="https://github.com/ebibibi/obsidian-cjk-bold-fix" target="_blank" class="card">
  <div class="card-thumbnail">
    <img src="/images/oss-obsidian-cjk-bold-fix.png" alt="CJK Bold Fix">
  </div>
  <div class="card-content">
    <h3>Obsidian CJK Bold Fix</h3>
    <p>ObsidianのLive Previewで日本語の太字・斜体が正しく表示されないCommonMarkの問題を修正するプラグイン。</p>
  </div>
</a>

<a href="https://github.com/ebibibi/ffmpeg-multistream-azure" target="_blank" class="card">
  <div class="card-thumbnail">
    <img src="/images/oss-ffmpeg-multistream.png" alt="ffmpeg-multistream-azure">
  </div>
  <div class="card-content">
    <h3>ffmpeg-multistream-azure</h3>
    <p>Azure Container InstancesでYouTube・Facebook・X・LinkedInに同時配信。使った分だけ課金。</p>
  </div>
</a>

<a href="https://github.com/ebibibi/restreamer-azure" target="_blank" class="card">
  <div class="card-thumbnail">
    <img src="/images/oss-restreamer-azure.png" alt="restreamer-azure">
  </div>
  <div class="card-content">
    <h3>restreamer-azure</h3>
    <p>Azure Container InstancesでRestreamerを必要な時だけ起動。設定は自動バックアップ・リストア。</p>
  </div>
</a>

<a href="https://github.com/ebibibi/ansible-hyperv" target="_blank" class="card">
  <div class="card-thumbnail">
    <img src="/images/oss-ansible-hyperv.png" alt="ansible-hyperv">
  </div>
  <div class="card-content">
    <h3>ansible-hyperv</h3>
    <p>AnsibleでHyper-V上のVMを自動プロビジョニングするPlaybook集。検証環境の構築を自動化。</p>
  </div>
</a>

<a href="/github-projects" class="card">
  <div class="card-thumbnail card-thumbnail-github">
    <svg viewBox="0 0 24 24" fill="currentColor" width="80" height="80"><path d="M12 0C5.374 0 0 5.373 0 12c0 5.302 3.438 9.8 8.207 11.387.599.111.793-.261.793-.577v-2.234c-3.338.726-4.033-1.416-4.033-1.416-.546-1.387-1.333-1.756-1.333-1.756-1.089-.745.083-.729.083-.729 1.205.084 1.839 1.237 1.839 1.237 1.07 1.834 2.807 1.304 3.492.997.107-.775.418-1.305.762-1.604-2.665-.305-5.467-1.334-5.467-5.931 0-1.311.469-2.381 1.236-3.221-.124-.303-.535-1.524.117-3.176 0 0 1.008-.322 3.301 1.23A11.509 11.509 0 0 1 12 5.803c1.02.005 2.047.138 3.006.404 2.291-1.552 3.297-1.23 3.297-1.23.653 1.653.242 2.874.118 3.176.77.84 1.235 1.911 1.235 3.221 0 4.609-2.807 5.624-5.479 5.921.43.372.823 1.102.823 2.222v3.293c0 .319.192.694.801.576C20.566 21.797 24 17.3 24 12c0-6.627-5.373-12-12-12z"/></svg>
  </div>
  <div class="card-content">
    <h3>GitHub プロジェクト一覧</h3>
    <p>上記以外も含む全OSSプロジェクトの一覧です。</p>
  </div>
</a>

</div>

<h2 class="section-heading">趣味・日常</h2>

<div class="card-grid">

<a href="https://diary.ebisuda.net" target="_blank" class="card">
  <div class="card-thumbnail">
    <img src="/images/diary.png" alt="日記">
  </div>
  <div class="card-content">
    <h3>えび日記</h3>
    <p>昔からずっと書いている日記です。日々の出来事や思ったことなどを書いています。</p>
  </div>
</a>

<a href="https://www.youtube.com/channel/UCgKek_Bu1t_gJ83ABfLBwBQ" target="_blank" class="card">
  <div class="card-thumbnail">
    <img src="/images/youtube-music.png" alt="音楽">
  </div>
  <div class="card-content">
    <h3>胡田昌彦の音楽チャンネル</h3>
    <p>自分の音楽の成長記録を記録しているチャンネルです。エレキベース、ドラム、自作曲など。</p>
  </div>
</a>

<a href="https://www.youtube.com/channel/UCb2h5Jqh3SWOVwV8AUUgPpw" target="_blank" class="card">
  <div class="card-thumbnail">
    <img src="/images/youtube-shogi.png" alt="将棋">
  </div>
  <div class="card-content">
    <h3>胡田昌彦が将棋をのんびりやります</h3>
    <p>将棋は趣味としてのんびり毎日やってます。その様子を動画に記録しています。</p>
  </div>
</a>

<a href="https://soundcloud.com/masahiko-ebisuda" target="_blank" class="card">
  <div class="card-thumbnail">
    <img src="/images/soundcloud.png" alt="SoundCloud">
  </div>
  <div class="card-content">
    <h3>SOUNDCLOUD</h3>
    <p>作曲した曲はSOUNDCLOUDで公開しています。</p>
  </div>
</a>

</div>
